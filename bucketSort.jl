function bucketSort!(A)
    ...
    for i in 1:n                                #petlja za svrstavanje clanova niza u zavisnosti od range (tako bucketSort funckionise)
                                                #Int nam treba jer indeks mora biti tog tipa, a desni deo kao rezultat daje float
        ...
        push!(B[Int(ceil(A[i]/10)) ], A[i])     #kada smo odredili indeks, odnosno bucket u koji ide trenutni clan, smestamo ga u B[i]
    end
...
function insertionSort!(A)                      #obican insertionSort sa jednom izmenom:
    l = length(A)
    if l > 1                                    #ubacujemo proveru da li je bucket duzi od 1 clana, ako nije znaci da je ili prazan ili ima samo jednog clana - u oba slucaja sortiranje je suvisno
        for j in 2:l
            key = A[j]
            i = j - 1
            while(i>0 && key<A[i])   
